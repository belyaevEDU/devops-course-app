...
Нам выделенная инфраструктура от Yadro на время курса стояла за одним внешним IP и моя локальная инфраструктура стоит за CGNAT. Как следствие, для получения запросов из открытой сети требуется reverse proxy. Для этого был использован проект [frp](https://github.com/fatedier/frp). Манифесты для деплоймента frp client находятся [здесь](/k8s/frpc/).
