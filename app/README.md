...
Сервер работает, по умолчанию, на порту 8000. Отдельный сервер для метрик Prometheus работает, по умолчанию, на порту 9100.
...
1) Модуль [**requester**](/app/requester/README.md) - отправляет запросы в API ЦБ РФ
2) Модуль [**server**](/app/server/README.md) - слушает GET запросы и отвечает на них, вытягивая данные с **requester**. В [документации модуля](/app/server/README.md) описаны переменные окружения.
3) **main** - просто запускает [**server**](/app/server/README.md)
